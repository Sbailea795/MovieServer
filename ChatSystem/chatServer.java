...
public class chatServer {
    ...
    //unlimited storage potential for users. Hash table stores the CID as a key and the socket address as the value
    public static Hashtable<Integer, SocketAddress> IPmapping = new Hashtable<Integer, SocketAddress>();

    //Enters a loop until the user quits, checking for incoming traffic, retransmitting as necessary
    ...
    public static void main(String argv[]) throws Exception {
        ...
        Message reply = new Message(0, "NULL", "NULL");
        while(loop){
            try {
                ...
                if (datum.getStr1().equals("QUIT")){
                    ...
                    reply = new Message(cid, "QUIT", IPmapping.get(-datum.getCid()).toString());
                    ...
                    reply = new Message(index, "JOIN", IPmapping.get(index).toString());
                    repeatControlMessages(reply, socketConnection);
                    if (verbose) System.out.println("Sending: <" + reply.getCid() +", "+reply.getStr1()+", "+reply.getStr2()+">");
    ...
    ***********************************************/
    public static boolean repeatTextMessages(Message datum, DatagramSocket socket){
    ...
    public static boolean repeatControlMessages(Message datum, DatagramSocket socket){
        IPmapping.forEach((cid, hostName) -> {
            sendMessage(datum, socket, IPmapping.get(cid)); 
        });
        return true;
    ...
    *     returns true if successful               *
    ***********************************************/
    public static boolean sendMessage(Message datum, DatagramSocket socket, SocketAddress addr){
        try {
